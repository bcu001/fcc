...
const FormInput = ({ label, name, type = "text", register, errors, ...rest }) => (
...
const CreateCharacterPage = () => {
  ...
  const onSubmit = async (data) => {
    ...
    const payload = Object.keys(data).reduce((acc, key) => {
        ...
        if (["strength", "agility", "intelligence", "luck", "level"].includes(key)) {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        throw new Error(res.data?.message || `Failed to create character with status: ${res.status}`);
      }
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="my-4">
        <div id="charBox" className="flex flex-col md:flex-row gap-5 w-full">
          ...
          <div className="border shadow-lg flex-1 p-3 rounded flex flex-col gap-3">
            <div className="text-md font-bold border-b pb-2">Core Information</div>
            ...
            <div className="space-y-3 pt-4 border-t mt-auto">
              ...
              <div className="grid grid-cols-2 gap-3">
                <FormInput
                  ...
                  register={register}
                  errors={errors}
                  rules={{ min: { value: 1, message: "Min 1" }, valueAsNumber: true }}
                />
                <FormInput
          ...
          <div className="border shadow-lg flex-2 p-3 rounded flex flex-col gap-4">
            ...
            {/* Special Ability */}
            <div>
              <label htmlFor="special ability" className="font-semibold text-sm">
